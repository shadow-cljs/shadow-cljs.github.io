...
`:dependencies` is only used to configure JVM dependencies. JavaScript dependencies are handled via the standard `npm` `package.json`. See the <<JS Dependencies, JavaScript Dependencies>> section for more details.
