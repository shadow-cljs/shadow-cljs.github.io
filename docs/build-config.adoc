...
preloads option is simply a list of namespaces either in the `:devtools`/`:preloads` section of
`shadow-cljs.edn` or within the `:preloads` key of a specific module:
...
IMPORTANT: This may become slow for larger apps, only use it if you really need it.
...
This example would call `(my.util/hook build-state 1 2 3)` after the build completed the `:flush` <<compile-stages, stage>> (ie. written to disk). The example would print `[:hello-world (1 2 3)]` but please do something more useful in actual hooks.
