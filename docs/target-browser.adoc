...
By default the dev server will attempt to serve requests locally but sometimes you want to use an external web server to serve requests (eg. API request). This can be configured via `:proxy-url`.

```
{...
 :builds
 {:app {...
        :devtools {:proxy-url "https://some.host"
                   :http-root "public"
                   :http-port 8000
                   ...}}}}
```
