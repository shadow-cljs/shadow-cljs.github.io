...
When a `shadow-cljs` <<server-mode, server instance>> is running it also provides a nREPL server which tools can connect to. It uses a random port on startup by default and writes this port for `target/shadow-cljs/nrepl.port` on startup. You can also configure this port in `shadow-cljs.edn`
