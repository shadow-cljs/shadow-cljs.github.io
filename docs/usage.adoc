...
When executing directly via `lein` you must manually include the https://clojars.org/thheller/shadow-cljs[thheller/shadow-cljs] artifact in your `:dependencies`. This is automatically added when launching via `shadow-cljs`.
