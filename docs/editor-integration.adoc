...
Note that the Cursive REPL when first connected always starts out as a CLJ REPL. You should switch it to CLJS via the Cursive select box, and then select it by calling `(shadow.cljs.devtools.api/repl :your-build-id)`. You can type `:cljs/quit` to drop back down to the CLJ REPL.
