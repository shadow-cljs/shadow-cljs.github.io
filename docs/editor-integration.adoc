...
Create the example app by following the official https://github.com/thheller/shadow-cljs#quick-start[Shadow-CLJS Quick Start] guide.

=== Configurations

Once the app is created, modify its `shadow-cljs.edn` as follows:
```clojure
...
{:source-paths
 ...
 ;; ADD - a port (e.g., 3333) for nREPL server to which Fireplace.vim connects
 :nrepl
 {:port 3333}
